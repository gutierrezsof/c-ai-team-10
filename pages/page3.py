...
Page setup
    ...
    "Computer and Mathematical", but can be switched to other occupational groups
    while still allowing the user to switch back to "Computer and Mathematical".

AI assistance: Used Claude to implement code changes per the rules and design
of our page layout - Reviewed and helped with set up of default selection,
...
_group_values = {
    opt["value"]
    for opt in OCCUPATION_GROUP_OPTIONS
}
...
def filter_career_options(selected_group):
    ...
    if not selected_group or selected_group == "All":
        ...
        filtered_df = careers_df[
            careers_df["occupation_group"] == selected_group
        ]

    options = [
        {"label": title, "value": title}
        for title in sorted(
            filtered_df["national_occ_title"].unique()
        )
    ]

    # Add preferred careers if they actually exist in this group
    new_value = [
        career
        for career in PREFERRED_CAREERS
        if career in filtered_df["national_occ_title"].values
    ]

    # Fill remaining slots until there are 4 careers
    for opt in options:
        if (
            opt["value"] not in new_value
            and len(new_value) < 4
        ):
            new_value.append(opt["value"])
...
# define warnings for comparing charts
def update_comparison_charts(selected_careers, metric_toggle):
    ...
    if len(selected_careers) < 2:
        ...
        warning = (
            "Please select 5 or fewer careers. "
            "Showing the first 5 selected."
        )
    ...
    wage_fig = go.Figure(
        data=[
            go.Bar(
                ...
                text=filtered["national_median_wage"].map(
                    lambda v: f"${v:,.0f}"
                ),
